...
class OrdersMonitor:
    ...
    def __init__(self, binance_client, on_order_filled_callback: Optional[Callable] = None,
                 live_trading_manager=None, symbol: str = None,
        ...
        Args:
            ...
            symbol: 订阅实时成交价格的交易对（如 'BTCUSDT'），为 None 则不订阅价格
            on_price_update: 价格更新回调，接收 (price: float) 参数
        """
        ...
        # 已收盘 K 线订阅（实时策略使用）
        self.kline_symbol = symbol.lower() if symbol else None
        ...
        self._price_ws = None
        self._price_ws_thread = None
        self._price_reconnect_count = 0

        # 测试网与生产环境使用各自的 WebSocket 地址。
        if testnet:
            self.WS_USER_DATA_STREAM_URL = "wss://fstream.binancefuture.com/ws/"
            self.WS_MARKET_URL = "wss://fstream.binancefuture.com/ws/"
    ...
    def start(self):
        ...
        # 启动价格订阅（如果配置了 symbol）
        if self.price_symbol and self.on_price_update:
            self._start_price_stream()

        if self.kline_symbol and self.kline_interval and self.on_kline_closed:
            self._start_kline_stream()
        
        logger.info("✅ 订单监控WebSocket已启动")

    def start_market_only(self):
        """只启动公开行情流，不创建需要 API Key 的 User Data Stream。"""
        if self.running:
            return
        self.running = True
        if self.price_symbol and self.on_price_update:
        ...
        if self.kline_symbol and self.kline_interval and self.on_kline_closed:
            self._start_kline_stream()
        logger.info("✅ Binance 公开行情 WebSocket 已启动")
    ...
    def stop(self):
        ...
        self._stop_kline_stream()
        
        logger.info("✅ 订单监控已停止")
    
    # ------------------------------------------------------------------
    # 实时价格订阅（aggTrade stream）
    # ------------------------------------------------------------------

    def update_price_symbol(self, symbol: str, on_price_update: Optional[Callable[[float], None]] = None) -> None:
        """
        切换价格订阅的交易对。如果监控已在运行中，立即重启价格 WebSocket。

        Args:
            symbol: 新的交易对（如 'ETHUSDT'）
            on_price_update: 可选，更新价格回调；为 None 则保留旧回调
        """
        self._stop_price_stream()
        self.price_symbol = symbol.lower()
        if on_price_update is not None:
            self.on_price_update = on_price_update
        if self.running and self.price_symbol and self.on_price_update:
            self._start_price_stream()

    def _start_price_stream(self) -> None:
        """启动 aggTrade 价格订阅 WebSocket 线程。"""
        if not self.price_symbol:
            return
        self._price_reconnect_count = 0
        self._price_ws_thread = threading.Thread(
            target=self._run_price_websocket, daemon=True, name="price-ws"
        )
        self._price_ws_thread.start()
        logger.info(f"✅ 价格订阅已启动: {self.price_symbol}@aggTrade")

    def _stop_price_stream(self) -> None:
        """停止 aggTrade 价格订阅 WebSocket 线程。"""
        if self._price_ws:
            try:
                ...
                pass
            self._price_ws = None
        if self._price_ws_thread:
            self._price_ws_thread.join(timeout=2)
            self._price_ws_thread = None
    ...
    def _on_price_message(self, ws, message) -> None:
        ...
        try:
            ...
            logger.debug(f"价格消息解析异常: {exc}")

    def _run_price_websocket(self) -> None:
        """在独立线程中维护 aggTrade WebSocket，支持自动重连。"""
        url = f"{self.WS_MARKET_URL}{self.price_symbol}@aggTrade"
        max_attempts = 10

        while self.running and self._price_reconnect_count < max_attempts:
            try:
                self._price_ws = websocket.WebSocketApp(
                    url,
                    on_message=self._on_price_message,
                    on_error=lambda ws, e: logger.debug(f"价格WS错误: {e}"),
                    on_close=lambda ws, c, m: logger.debug("价格WS已关闭"),
                )
                run_kwargs = dict(ping_interval=self.ping_interval, ping_timeout=self.ping_timeout)
                ...
                self._price_ws.run_forever(**run_kwargs)
            except Exception as exc:
                logger.warning(f"价格WS运行异常: {exc}")

            if not self.running:
                break

            self._price_reconnect_count += 1
            delay = min(2 ** self._price_reconnect_count, 30)
            logger.info(f"价格WS重连中 (第{self._price_reconnect_count}次)，{delay}s 后重试...")
            time.sleep(delay)
    ...
    def _start_kline_stream(self) -> None:
        """启动指定 ticker/interval 的合约 K 线 WebSocket。"""
        if not self.kline_symbol or not self.kline_interval:
            return
        self._kline_reconnect_count = 0
        self._kline_ws_thread = threading.Thread(
            target=self._run_kline_websocket, daemon=True, name="kline-ws")
        self._kline_ws_thread.start()
        logger.info(
            f"✅ K线订阅已启动: {self.kline_symbol}@kline_{self.kline_interval}")

    def _stop_kline_stream(self) -> None:
        if self._kline_ws:
            try:
    ...
    def _run_kline_websocket(self) -> None:
        """沿用本框架的代理、PING/PONG 与指数退避重连机制。"""
        url = f"{self.WS_MARKET_URL}{self.kline_symbol}@kline_{self.kline_interval}"
        while self.running and self._kline_reconnect_count < self.max_reconnect_attempts:
            try:
                self._kline_ws = websocket.WebSocketApp(
                    url,
                    on_message=self._on_kline_message,
                    on_error=lambda ws, e: logger.warning(f"K线WS错误: {e}"),
                    on_close=lambda ws, c, m: logger.info("K线WS已关闭"),
                )
                run_kwargs = dict(
                ...
                self._kline_ws.run_forever(**run_kwargs)
            except Exception as exc:
                logger.warning(f"K线WS运行异常: {exc}")

            if not self.running:
                break
            self._kline_reconnect_count += 1
            delay = min(2 ** self._kline_reconnect_count, 30)
            logger.info(
                f"K线WS重连中 (第{self._kline_reconnect_count}次)，{delay}s 后重试...")
            time.sleep(delay)
    ...
    def _handle_order_update(self, order_data: dict):
        ...
        if self.on_order_update_callback:
            try:
                self.on_order_update_callback(copy.deepcopy(order_data))
            except Exception as exc:
                logger.warning(f"订单状态回调异常: {exc}")
    ...
    def _place_take_profit_order(self, order_id: str, symbol: str, signal_type: str, 
        ...
        try:
            take_profit_result = self.binance_client.place_take_profit_limit_order(
                ...
                stop_price=take_profit_trigger_price,
            ...
            if take_profit_result is not None and not take_profit_result.get('error'):
                ...
                if not take_profit_order_id:
                    logger.error("❌ 限价止盈单返回缺少 orderId: %s", take_profit_result)
                    return None
                ...
                logger.info(f"✅ 限价止盈单已提交: order_id={take_profit_order_id}, price={take_profit_trigger_price:.2f}, quantity={executed_qty}, 触发信号订单ID: {order_id}")
