...
def load_runtime_env(override: bool = True) -> None:
    """兼容原 Client 模块的环境加载入口。"""
    load_dotenv(os.path.join(APP_DIR, ".env"), override=override)
...
class BinanceClient:
    ...
    def __init__(
        ...
        self.secret_key = self._normalize_credential(
            secret_key or os.getenv('BINANCE_API_SECRET', '')
            or os.getenv('BINANCE_SECRET_KEY', ''))
        self.testnet = testnet
        default_base_url = 'https://testnet.binancefuture.com' \
            if testnet else 'https://fapi.binance.com'
        self.base_url = base_url or os.getenv('BINANCE_BASE_URL', '').strip() \
            or default_base_url
        ...
        if not self.api_key or not self.secret_key:
            raise ValueError(
                ...
                "2. 在 .env 或 .env.enc 文件中设置：BINANCE_API_KEY=... 和 BINANCE_SECRET_KEY=..."
            )
    ...
    def get_server_time(self) -> Optional[datetime]:
        ...
        try:
            ...
            server_time = datetime.fromtimestamp(server_time_ms['serverTime'] / 1000.0, tz=timezone.utc)
            return server_time
        except Exception as e:
    ...
    def get_multi_assets_mode(self) -> Optional[bool]:
        """Get margin asset mode: True = Multi-Assets Mode, False = Single-Asset Mode."""
        try:
            self.set_timestamp_offset(force=True)
            result = self.client.futures_get_multi_assets_mode()
            return result.get('multiAssetsMargin', False)
        except Exception as e:
            logger.debug(f"Failed to get multi assets mode: {e}")
            return None
    ...
    def place_market_order(self, symbol: str, side: str, quantity: float, position_side: str = None, reduce_only: bool = False) -> Optional[dict]:
        ...
        for attempt in range(self.MAX_TIMESTAMP_RETRIES + 1):
            try:
                ...
                normalized_position_side = str(position_side or "").upper() or None
                ...
                if position_mode is True:
                    ...
                    # One-way Mode: Binance expects BOTH rather than LONG/SHORT.
    ...
    def place_close_all_stop_loss_order(self, symbol: str, side: str, stop_price: float) -> Optional[dict]:
        ...
        Binance requires closePosition=true conditional orders to be sent to the
        algo order endpoint and to omit quantity.
        """
        try:
            position_mode = self.get_position_mode()
            if position_mode is True:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        error_text = response.text
                        logger.debug(f"❌ 全仓止损 Algo Order API 错误 (status={response.status_code}): {error_text}")
                        return None
                except Exception as e:
                    last_exception = e
                    if attempt < max_retries:
                        wait_time = 2 ** attempt
                        logger.debug(f"⏳ 下全仓止损单异常，等待 {wait_time} 秒后重试...")
                        time.sleep(wait_time)
                        continue
                    break
            ...
            logger.debug(f"Failed to place close-all stop loss order for {symbol}: {e}")
            if 'params' in locals():
                logger.debug(f"  请求参数: {params}")
    ...
    def place_close_all_take_profit_order(self, symbol: str, side: str, trigger_price: float) -> Optional[dict]:
        ...
        Binance requires closePosition=true conditional take-profit orders to be sent to the
        algo order endpoint and to omit quantity.
        """
        try:
            position_mode = self.get_position_mode()
            ...
            logger.debug(f"Failed to place close-all take profit order for {symbol}: {e}")
            if 'params' in locals():
                logger.debug(f"  请求参数: {params}")
    ...
    def has_open_position(self, symbol: str) -> Optional[bool]:
        """查询指定交易对是否有仓位；请求失败时返回 None，避免误开仓。"""
        try:
            positions = self.client.futures_position_information(symbol=symbol)
            return any(
    ...
    def get_positions(self, symbol: str | None = None) -> List[dict]:
        ...
        Prefer /fapi/v3/positionRisk so UI fields match Binance's current
        position view as closely as possible.
        """
        try:
            import requests

            current_position_mode = self.get_position_mode()

            def _position_mode_label() -> str:
                if current_position_mode is True:
                    return "hedge"
                if current_position_mode is False:
                    return "one_way"
                return ""
            ...
            def _position_key(row: dict) -> tuple[str, str]:
                ...
                if position_side in ("", "BOTH"):
                    ...
                    if position_amt > 0:
                        ...
                        position_side = "SHORT"
                return symbol_key, position_side

            account_position_index: dict[tuple[str, str], dict] | None = None

            def _get_account_position_index() -> dict[tuple[str, str], dict]:
                nonlocal account_position_index
                if account_position_index is not None:
                    return account_position_index

                account_position_index = {}
                try:
                    try:
                    ...
                    for raw in (account or {}).get("positions", []) or []:
                        account_position_index[_position_key(raw)] = dict(raw)
                except Exception as exc:
                    logger.debug(f"Failed to fetch account position metadata: {exc}")
                return account_position_index

            def _infer_margin_type(row: dict) -> str:
                isolated = row.get("isolated")
                if isinstance(isolated, bool):
                    return "isolated" if isolated else "cross"
                isolated_text = str(isolated or "").strip().lower()
                if isolated_text in ("true", "1"):
                    return "isolated"
                if isolated_text in ("false", "0"):
                    return "cross"
                return ""

            def _normalize_position_row(raw_row: dict) -> dict:
                row = dict(raw_row)
                ...
                position_mode = _position_mode_label()
                if position_mode:
                    row["positionMode"] = position_mode

                if not (_has_value(row.get("leverage")) and _has_value(row.get("marginType"))):
                    account_row = _get_account_position_index().get(_position_key(row), {})
                    if account_row:
                        if not _has_value(row.get("leverage")) and _has_value(account_row.get("leverage")):
                            row["leverage"] = account_row.get("leverage")
                        if not _has_value(row.get("marginType")) and _has_value(account_row.get("marginType")):
                            row["marginType"] = account_row.get("marginType")
                        if not _has_value(row.get("isolated")) and _has_value(account_row.get("isolated")):
                            row["isolated"] = account_row.get("isolated")

                if not _has_value(row.get("marginType")):
                    inferred_margin_type = _infer_margin_type(row)
                    if inferred_margin_type:
                        row["marginType"] = inferred_margin_type

                return row

            def _fetch_position_risk(path: str) -> List[dict]:
                ...
                for r in rows:
                    if float(r.get("positionAmt", 0) or 0) == 0:
                        continue
                    filtered.append(_normalize_position_row(r))
                return filtered

            try:
                return _fetch_position_risk("/fapi/v3/positionRisk")
            except Exception as v3_exc:
                logger.debug(f"Failed to get positions from v3 positionRisk: {v3_exc}")
                return _fetch_position_risk("/fapi/v2/positionRisk")
        except Exception as e:
    ...
    def get_account_margin_ratio(self) -> Optional[float]:
        ...
        try:
            ...
            # Binance Futures 账户响应已提供汇总维持保证金和保证金余额；
            # 优先使用汇总值，避免 positionRisk 响应不含 maintMargin 时误算为 0。
            total_maint = account.get('totalMaintMargin')
            total_equity = account.get('totalMarginBalance')
            if total_maint not in (None, '') and total_equity not in (None, ''):
                maintenance = float(total_maint or 0)
            ...
            if equity > 0:
                ...
                return margin_ratio
            return 0.0 if total_maint_margin == 0 else None
        except Exception as e:
